...
LOCALIZATION = {
    'ru': {
        'welcome': WELCOME_TEXT, # Используем ваш текст
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if 'language' not in context.user_data:
        ...
        # Используем ваш новый текст
        # Берем его прямо из переменной WELCOME_TEXT, чтобы он всегда был одинаковым при первом запуске
...
async def change_language(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # При смене языка показываем тот же приветственный экран с выбором
    await send_clean_message(
        update,
        context,
        WELCOME_TEXT,
